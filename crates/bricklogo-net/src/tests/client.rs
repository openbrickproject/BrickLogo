...
fn start_mock_host(port: u16, initial_vars: HashMap<String, LogoValue>) -> (
    ...
    let client_handle: Arc<Mutex<Option<TcpStream>>> = Arc::new(Mutex::new(None));
            ...
            *handle_clone.lock().unwrap() = Some(writer);
            ...
            loop {
                ...
                match protocol::decode(&line) {
                    Ok(NetMessage::Sync) => {
                        let snapshot = protocol::encode(&NetMessage::Snapshot {
                            vars: initial_vars.clone(),
                        });
                        if let Some(w) = handle_clone.lock().unwrap().as_mut() {
                            let _ = w.write_all(snapshot.as_bytes());
                            let _ = w.flush();
                        }
                    }
...
fn test_client_receives_remote_set() {
    ...
    if let Some(ref mut w) = *handle.lock().unwrap() {
        let msg = protocol::encode(&NetMessage::Set {
