...
pub fn register_hardware_primitives(
    ...
    eval.register_primitive("connect", PrimitiveSpec {
        ...
        func: Box::new(move |args, _| {
            ...
            fn next_config_entry(list: &[String], indices: &mut std::collections::HashMap<String, usize>, key: &str) -> Option<String> {
    ...
    let pm_ref = pm.clone();
    eval.register_primitive("disconnect", PrimitiveSpec {
    ...
    let pm_ref = pm.clone();
    eval.register_primitive("use", PrimitiveSpec {
        min_args: 1, max_args: 1,
        func: Box::new(move |args, _| {
            let name = args[0].as_string().to_lowercase();
            pm_ref.lock().unwrap().set_active_device(&name).map_err(|e| LogoError::Runtime(e))?;
            Ok(None)
        }),
    });
    ...
    let pm_ref = pm.clone();
    eval.register_primitive("talkto", PrimitiveSpec {
        min_args: 1, max_args: 1,
        func: Box::new(move |args, _| {
            let mut pm = pm_ref.lock().unwrap();
    ...
    eval.register_primitive("on", PrimitiveSpec {
        ...
        func: Box::new(move |_, _| {
            pm_ref.lock().unwrap().on().map_err(|e| LogoError::Runtime(e))?;
            Ok(None)
        }),
    });
    ...
    eval.register_primitive("off", PrimitiveSpec {
        min_args: 0, max_args: 0,
        func: Box::new(move |_, _| {
            pm_ref.lock().unwrap().off().map_err(|e| LogoError::Runtime(e))?;
            Ok(None)
        }),
    });
    ...
    eval.register_primitive("onfor", PrimitiveSpec {
        ...
        func: Box::new(move |args, _| {
            ...
            pm_ref.lock().unwrap().on_for(tenths).map_err(|e| LogoError::Runtime(e))?;
            Ok(None)
        }),
    });
    ...
    eval.register_primitive("rotate", PrimitiveSpec {
        ...
        func: Box::new(move |args, _| {
            ...
            pm_ref.lock().unwrap().rotate(degrees).map_err(|e| LogoError::Runtime(e))?;
            Ok(None)
        }),
    });
    ...
    eval.register_primitive("rotateto", PrimitiveSpec {
        ...
        func: Box::new(move |args, _| {
            ...
            pm_ref.lock().unwrap().rotate_to(position).map_err(|e| LogoError::Runtime(e))?;
            Ok(None)
        }),
    });
    ...
    eval.register_primitive("resetzero", PrimitiveSpec {
        min_args: 0, max_args: 0,
        func: Box::new(move |_, _| {
            pm_ref.lock().unwrap().reset_zero().map_err(|e| LogoError::Runtime(e))?;
            Ok(None)
        }),
    });
    ...
    eval.register_primitive("rotatetohome", PrimitiveSpec {
        min_args: 0, max_args: 0,
        func: Box::new(move |_, _| {
            pm_ref.lock().unwrap().rotate_to_home().map_err(|e| LogoError::Runtime(e))?;
            Ok(None)
        }),
    });
    ...
    eval.register_primitive("sensor", PrimitiveSpec {
        ...
        func: Box::new(move |args, _| {
            ...
            let val = pm_ref.lock().unwrap().read_sensor(Some(&mode))
    ...
    eval.register_primitive("sensor?", PrimitiveSpec {
        min_args: 0, max_args: 0,
        func: Box::new(move |_, _| {
            match pm_ref.lock().unwrap().read_sensor(None) {
                Ok(Some(val)) => {
    ...
    // ── File primitives (stubs — actual impl in app) ──

    eval.register_primitive("namepage", PrimitiveSpec {
        min_args: 1, max_args: 1,
        func: Box::new(|_, _| Ok(None)), // handled by app
    });
    eval.register_alias("np", "namepage");

    eval.register_primitive("save", PrimitiveSpec {
        min_args: 0, max_args: 0,
        func: Box::new(|_, _| Ok(None)), // handled by app
    });

    eval.register_primitive("load", PrimitiveSpec {
        min_args: 1, max_args: 1,
        func: Box::new(|_, _| Ok(None)), // handled by app
    });
    ...
    eval.register_primitive("setdisk", PrimitiveSpec {
        min_args: 1, max_args: 1,
        func: Box::new(|_, _| Ok(None)), // handled by app
    });

    eval.register_primitive("disk", PrimitiveSpec {
        min_args: 0, max_args: 0,
        func: Box::new(|_, _| Ok(None)), // handled by app
    });

    eval.register_primitive("erase", PrimitiveSpec {
        min_args: 1, max_args: 1,
        func: Box::new(|_, _| Ok(None)), // handled by app
    });
