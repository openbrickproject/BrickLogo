...
mod tests {
    ...
    fn parse_str(input: &str) -> Vec<AstNode> {
        ...
        // Register some known arities for testing
        parser.set_arity("print", 1);
        parser.set_arity("sum", 2);
        parser.set_arity("make", 2);
        parser.set_arity("repeat", 1); // repeat is a special form but just for arity lookup
    ...
    fn test_procedure_def() {
        ...
        assert!(matches!(&ast[0], AstNode::ProcDef { name, params, body }
            if name == "greet" && params.len() == 1 && body.len() == 1));
    }
    ...
    fn test_recursive_procedure() {
        ...
        parser.set_arity("print", 1);
        let ast = parser.parse(tokens).unwrap();
        assert!(matches!(&ast[0], AstNode::ProcDef { name, params, .. }
            if name == "countdown" && params.len() == 1));
    }
}
