...
fn node_to_source(node: &AstNode) -> String {
    match node {
        ...
        AstNode::List(elements) => {
            let inner = elements.iter().map(node_to_source).collect::<Vec<_>>().join(" ");
            format!("[{}]", inner)
        }
        AstNode::Infix { operator, left, right } => {
            format!("{} {} {}", node_to_source(left), operator, node_to_source(right))
        }
        AstNode::Paren { name, args } => {
            let arg_strs = args.iter().map(node_to_source).collect::<Vec<_>>().join(" ");
        ...
        AstNode::ProcDef { name, params, body } => {
            procedure_to_source(&UserProcedure { name: name.clone(), params: params.clone(), body: body.clone() })
        }
        AstNode::Repeat { count, body } => {
            format!("repeat {} [{}]", node_to_source(count), body_to_source(body))
        }
        AstNode::Forever { body } => {
            format!("forever [{}]", body_to_source(body))
        }
        AstNode::If { condition, body } => {
            format!("if {} [{}]", node_to_source(condition), body_to_source(body))
        }
        AstNode::IfElse { condition, then_body, else_body } => {
            format!("ifelse {} [{}] [{}]", node_to_source(condition), body_to_source(then_body), body_to_source(else_body))
        }
        AstNode::WaitUntil { condition } => {
            format!("waituntil [{}]", body_to_source(condition))
        }
        AstNode::Carefully { body, handler } => {
            format!("carefully [{}] [{}]", body_to_source(body), body_to_source(handler))
        }
...
fn body_to_source(body: &[AstNode]) -> String {
    body.iter().map(node_to_source).collect::<Vec<_>>().join(" ")
}
...
pub fn procedure_to_source(proc: &UserProcedure) -> String {
    ...
    let body = proc.body.iter().map(|n| format!("  {}", node_to_source(n))).collect::<Vec<_>>().join("\n");
    format!("{}\n{}\nend", header, body)
}

pub fn procedures_to_source(procs: &[&UserProcedure]) -> String {
    let sources = procs.iter().map(|p| procedure_to_source(p)).collect::<Vec<_>>();
    format!("{}\n", sources.join("\n\n"))
}
...
mod tests {
    ...
    fn test_repeat() {
        let mut eval = create_evaluator();
        eval.evaluate("to square :n repeat 4 [print :n] end").unwrap();
        let procs = eval.get_all_procedures();
        let source = procedure_to_source(procs[0]);
    ...
    fn test_ifelse() {
        let mut eval = create_evaluator();
        eval.evaluate("to check :x ifelse :x > 0 [print \"pos] [print \"neg] end").unwrap();
        let procs = eval.get_all_procedures();
        let source = procedure_to_source(procs[0]);
    ...
    fn test_round_trip() {
        ...
        eval1.evaluate("to countdown :n if :n = 0 [print \"done stop] print :n countdown :n - 1 end").unwrap();
